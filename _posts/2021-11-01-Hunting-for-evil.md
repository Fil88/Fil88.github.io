...
### 9) Detects malicious SMB Named Pipe
...
let badPipeNames = pack_array(
    ...
    '\\Posh',                                       
	// PoshC2 default named pipe
    '\\mojo.*',
    '\\crashpad_'
);
DeviceEvents
| where InitiatingProcessFolderPath !contains "msedge.exe" and InitiatingProcessFolderPath !contains "msedgewebview2.exe" and InitiatingProcessFolderPath !contains "notification_helper.exe" and InitiatingProcessFolderPath !contains "chrome"
...
### 11) Find potentially malicious .jse launch by File Explorer, Word, Excel or Outlook 

```powershell
// Find potentially malicious .jse launch by File Explorer, Word, Excel or Outlook 
DeviceProcessEvents 
| where Timestamp > ago(30d) 
| where InitiatingProcessFileName in~ ('explorer.exe','winword.exe', 'excel.exe', 'outlook.exe') 
and FileName =~ "wscript.exe" and ProcessCommandLine contains ".js" or ProcessCommandLine contains ".jse"
